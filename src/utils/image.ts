...
export const queryParamsToOptions = (ctx: Context) => {
	...
	for (let key in optionsMap) {
		...
		if (val || typeof val === 'string') {
			...
			console.log(
				`[${group}]${type ? `[${type}]` : '[       ]'} ${JSON.stringify(key, null, 2)} => ${JSON.stringify(
					val,
					null,
					2
				)}`
			...
			if (!isLongPathName(key)) {
				// short

				options[group] = val
			} else {
				// long, like 'modulate.hue'
...
export const isNeedResize = (ctx: Context): boolean => {
	...
	// nothing set
	if (
		!('width' in query && query.width) &&
		!('height' in query && query.height) &&
		!('w' in query && query.w) &&
		!('h' in query && query.h)
	) {
		return false
	...
	if (
		...
		if (
			(width === query.width || width === query.w) &&
			(height === query.height || height === query.h)
		) {
			return false
		} else {
			return true
...
export const convertType = (
	type: string,
	value: string
): string | number | Array<any> | boolean => {
	if (type === 'integer') {
